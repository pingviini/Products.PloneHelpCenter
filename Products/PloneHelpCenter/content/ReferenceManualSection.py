...
HelpCenterReferenceManualSectionSchema = folder.ATFolderSchema.copy()
HideMetadataFields(HelpCenterReferenceManualSectionSchema)


class HelpCenterReferenceManualSection(folder.ATFolder):
    ...
    typeDescription = 'A Reference Manual Section can contain Reference '\
                      'Manual Pages, and other Reference Manual '\
                      '(Sub-)Sections. Index order is decided by the folder '\
                      'order, use the normal up/down arrow in the folder '\
                      'content view to rearrange content.'
    ...
    def getSectionDescription(self):
        """ Returns the description of the section -- convenience method for
        ReferenceManualPage
        """
    ...
    def contentIds(self, filter=None):
        """ List IDs of contentish and folderish sub-objects. (method is
        without docstring to disable publishing)
